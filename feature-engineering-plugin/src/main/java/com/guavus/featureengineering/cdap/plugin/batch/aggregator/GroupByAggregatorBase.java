...
public abstract class GroupByAggregatorBase extends RecordAggregator {
	...
	public void aggregate(StructuredRecord groupKey, Iterator<StructuredRecord> iterator,
		...
		if (isDynamicSchema) {
			Schema dynamicOutoutSchema = getDynamicOutputSchema(inputSchema, conf.getGroupByFields(), conf.getAggregates(),
					aggregateFunctions);
		...
		for (Map.Entry<String, AggregateFunction> aggregateFunction : aggregateFunctions.entrySet()) {
			...
			if(result instanceof Map) {
				...
				for (String dict2 : valueSpecificFrequencyCount.keySet()) {
					...
					if(result instanceof Map) {
						Map<String, Integer> frequencyCount = (Map<String, Integer>) valueSpecificFrequencyCount.get(dict2);
						for (String dict1 : frequencyCount.keySet()) {
						...
						builder.set(aggregateFunction.getKey() + "_" + dict2.toLowerCase(), valueSpecificFrequencyCount.get(dict2));
					}
				} 
			} else {
	...
	protected Schema initAggregates(Schema inputSchema) {
		...
		if (!isDynamicSchema) {
			return getStaticOutputSchema(inputSchema, groupByFields, functionInfos,
					categoricalDictionaryMap, aggregateFunctions, functionInfoMap);
		} else {
