 ...
 */
public class JoinerDynamicConfig extends PluginConfig {
  ...
  private final String keysToBeAppended;
  
  @Nullable
  @Description(REQUIRED_INPUTS_DESC)
  protected String requiredInputs;

  public JoinerDynamicConfig() {
    this.joinKeys = "";
    this.selectedFields = "";
    this.requiredInputs = null;
    this.categoricalColumnsToBeChecked = "";
    this.keysToBeAppended = "";
  }

  @VisibleForTesting
  JoinerDynamicConfig(String joinKeys, String selectedFields, String requiredInputs, String categoricalColumnsToBeChecked, String keysToBeAppended) {
  ...
  List<String> getCategoricalColumnsToBeChecked() {
		...
		if (this.categoricalColumnsToBeChecked != null && !this.categoricalColumnsToBeChecked.isEmpty() && !this.categoricalColumnsToBeChecked.equals("NA")) {
			String tokens[] = categoricalColumnsToBeChecked.split(",");
			for (String token : tokens) {
				if(token.trim().isEmpty())
					continue;
				categoricalColumnsToBeCheckedList.add(token.trim());
			}
		}
		return categoricalColumnsToBeCheckedList;
	}

  Map<String, String> getKeysToBeAppended() {
