 ...
 */
public class GroupByCategoricalConfig extends AggregatorConfig {
	...
	@Description("Multi-column Aggregates to compute on grouped records. "
			+ "Supported aggregate functions are valuecount, indicatorcount, nuniq. "
			...
			+ "For example, 'tagcount:valuecount(tag),distinct:nuniq(tag)' will calculate two aggregates. "
			+ "The first will create a field called 'tagcount' that is the count of all 'tag' field values in the group. "
			+ "The second will create a field called 'distinct' that contains the count of unique values of 'tag' field in the group")
	...
	public GroupByCategoricalConfig() {
		this.groupByFields = "";
		this.aggregates = "";
	...
	@VisibleForTesting
	GroupByCategoricalConfig(String groupByFields, String aggregates, String categoricalDictionary) {
	 ...
	 */
	public List<String> getCategoricalDictionary() {
		...
		for (String field : Splitter.on(',').trimResults().split(categoricalDictionary)) {
			categoricalDictionaryFields.add(field);
		}
		if (categoricalDictionaryFields.isEmpty()) {
			throw new IllegalArgumentException("The 'categoricalDictionary' property must be set.");
		}
		return categoricalDictionaryFields;
	}
	...
	static class FunctionInfo {
		...
		public AggregateFunction getAggregateFunction(Schema fieldSchema) {
			switch (function) {
			case VALUECOUNT:
				return new ValueCount(field, fieldSchema);
			case INDICATORCOUNT:
				return new IndicatorCount(field, fieldSchema);
			}
			// should never happen
